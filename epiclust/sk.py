...
def sinkhorn_knopp(A, rowSums=None, colSums=None, epsilon=1e-5, max_iter=1000, verbose=True):
        import numpy as np
        import scipy.sparse
        if rowSums is None:
...
def biwhiten(adata, key="sk", n_comps=None, verbose=True, max_iter=1000, epsilon=1e-5):
    import numpy as np
    import scipy.sparse
    import scanpy as sc
    x, y = sinkhorn_knopp(adata.X, verbose=verbose, max_iter=max_iter, epsilon=epsilon)
    adata.obs["sk"] = x
    adata.var["sk"] = y
    X = adata.X.copy()
    adata.X = scipy.sparse.diags(np.sqrt(x)).dot(X).dot(scipy.sparse.diags(np.sqrt(y))).astype(np.float32)
