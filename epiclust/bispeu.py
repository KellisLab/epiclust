...
def val_to_ptr(typingctx, data):
    def impl(context, builder, signature, args):
        ...
        return ptr
    sig = types.CPointer(numba.typeof(data).instance_type)(
        numba.typeof(data).instance_type)
    return sig, impl
...
_bispeu_functype = ctypes.CFUNCTYPE(ctypes.c_void_p,
                                    ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_longlong), ctypes.POINTER(
                                        ctypes.c_double), ctypes.POINTER(ctypes.c_longlong),
...
BISPEU = _bispeu_functype(get_f2py_function_address(
    scipy.interpolate.dfitpack.bispeu._cpointer))
